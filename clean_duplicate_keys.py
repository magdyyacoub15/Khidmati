...
for lang, trans_dict in translations.items():
    ...
    good_block = "\n"
    # ONLY inject the visit_message because the others existed elsewhere
    val = trans_dict['confirm_delete_visit_message'].replace("'", "\\'")
    good_block += f"      'confirm_delete_visit_message': '{val}',\n"

    if bad_block in content:
        content = content.replace(bad_block, good_block)
        count += 1
        print(f"Fixed duplicate block for {lang}")

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"Done. Cleaned {count} languages.")
