...
for lang, trans_dict in translations.items():
    if "'confirm_delete_title'" in content and f"'{lang}':" in content.split("'confirm_delete_title'")[0]:
        # just a rudimentary check to avoid double injection, though false positives in giant file might occur
        pass
    
    target_str = f"    '{lang}': {{\n"
    if target_str in content and "'confirm_delete_title'" not in content.split(target_str)[1][:1000]:
        injection = target_str
        for key, val in trans_dict.items():
            val = val.replace("'", "\\'")
            injection += f"      '{key}': '{val}',\n"
        
        content = content.replace(target_str, injection, 1)
